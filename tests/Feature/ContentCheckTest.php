...
test('content check endpoint returns current versions', function () {
    ...
    $response
        ...
        ->assertJsonPath('versions.hadis_version', 1)
...
test('content check endpoint marks changed modules when client versions are behind', function () {
    ...
    $response = $this->getJson('/api/v1/content/check?zikir_version=1&dua_version=1&hadis_version=1&prayer_times_version=1');
